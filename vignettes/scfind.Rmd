...
If you have a matrix or a data frame containing expression data then you first need to create an `SingleCellExperiment` object containing your data. For illustrative purposes we will use an example expression matrix provided with `scfind`. The dataset (`Tabula Muris (FACS)`) represents __CPM__ gene expression of 865 and 981 cells derived from mouse liver and kidney, respectively. The investigators ([The Tabula Muris Consortium](https://doi.org/10.1038/s41586-018-0590-4)) have profiled every cell-type in the mouse using high-coverage FACS-sorted cells + Smartseq2. in the original publication (`kidney` and `marrow` data frames). We will combine the indices of `kidney` and `marrow` into a super-index later
...
# if the dataset contains more than one category, users can build index for each tissue individually and merge all indexes into 1 super-index using `mergeDataset` as following:
data(sce_tmfacs)
...
Once the index `scfind` index is built, one can view all existing genes in the datasets using `scfindGenes`
...
# The calculation above shows that a list of genes containing `Kynu`, `Nedd4l` and `Rab5b` is specific for the `Bone Marrow.Fraction A pre-pro B cell` cell type with the highest number of cell hits and total cells.
