...
```{r}
initialList <- c("Aif1", "4930578C19Rik", "Arhgef9", "9430076C15Rik", "C1qtnf2", "Ccdc109b")
...
enrichedCellTypes
# The calculation above shows that a list of genes containing `Aif1` and `Ccdc109b` is specific for the `Kidney.leukocyte` cell type with the lowest p-value.

# To further evaluate a specific query by calculating the precision recall metrics
evaluateGenes <- evaluateMarkers(geneIndex, bestQuery, "Kidney.leukocyte")
evaluateGenes
data1 <- t(evaluateGenes[, c(7,6,8)])
colnames(data1) <- evaluateGenes$genes
barplot(data1 , main = paste("Evaluation of markers in Kidney.leukocyte"), beside=T, legend=rownames(data1), ylab="Arbitrary unit", ylim = c(0, 1.1))
...
barplot(data2 , main = paste("Marker genes of ", interestedCellType), font.axis=1, beside=T, legend=rownames(data2), ylab="Arbitrary unit", ylim = c(0, 1.1))
